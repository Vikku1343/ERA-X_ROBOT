...
from Era_X_Robot import DRAGONS
from Era_X_Robot.modules.helper_funcs.telethn import IMMUNE_USERS, telethn
...
async def star_is_admin(chat_id: int):
    status = False
    star = await telethn.get_me()
    async for user in telethn.iter_participants(
        chat_id, filter=ChannelParticipantsAdmins
    ):
        if star.id == user.id:
